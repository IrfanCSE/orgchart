...
import { EdgeData, GraphData, NodeData } from "./AppState";
...
> {
  ...
  private updateContextMenuState(stateChange: Partial<ContextMenuState>): void {
    this.setState({
      contextMenu: { ...this.state.contextMenu, ...stateChange },
    });
  }
  ...
  private initializeTooltips() {
    ...
    inputMode.addQueryItemToolTipListener((src, eventArgs) => {
      ...
      eventArgs.toolTip = ReactGraphComponent.createTooltipContent(
        eventArgs.item!
      );
  ...
  componentDidUpdate(prevProps: ReactGraphComponentProps): void {
    if (
      this.props.graphData.nodesSource.length !==
      prevProps.graphData.nodesSource.length ||
      this.props.graphData.edgesSource.length !==
      prevProps.graphData.edgesSource.length
    ) {
